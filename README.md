...
新增 `POST /api/ehr/insights/retention-risk`，依据敬业度、薪酬差距、月度加班、近 90 天缺勤、晋升等待和司龄生成留任风险分值。接口仅返回业务解释和关怀建议，不使用姓名等无关个人属性，便于人事团队进行访谈、负荷调整和职业发展跟进。
...
`POST /api/ehr/insights/workforce-capacity` 按需求工时、可用工时、缺勤、加班与关键岗位空缺计算产能缺口和负荷风险，输出排班、备岗和招聘建议。该能力只服务团队产能规划，不用于自动化雇佣或人员淘汰决策。
